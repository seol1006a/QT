    ...
    , ui(new Ui::Tab3sqlite)
{
    ...
    QString strQuery = "create table sensor_tb ("
                       ...
                       "humi varchar(10))";
    QSqlQuery sqlQuery;
    //sqlQuery.exec("drop table sensor_tb");
...
void Tab3sqlite::updateLastDateTime(bool bFlag)
{
...
void Tab3sqlite::Tab3sqliteRecvDataSlot(QString recvData)
{
...
void Tab3sqlite::on_pPBsearchDB_clicked()
{
    ...
    if(sqlQuery.exec(strQuery))
    {
        ...
        if(pQTableWidgetItemId != nullptr)
        {
            ...
            pQTableWidgetItemTemp = nullptr;
            pQTableWidgetItemHumi = nullptr;
        }
        ...
        ui->pTBsensor->setColumnCount(5);
        QStringList headers;
        headers << "ID" << "날짜" << "조도" << "온도" << "습도";
        ui->pTBsensor->setHorizontalHeaderLabels(headers);

    }
}


void Tab3sqlite::on_pPBdeleteDB_clicked()
{
    QDateTime fromDateTime = ui->pDateTimeEditFrom->dateTime();
    QString strFromDateTime = fromDateTime.toString("yyyy/MM/dd hh:mm:ss");
    QDateTime toDateTime = ui->pDateTimeEditTo->dateTime();
...
void Tab3sqlite::on_pPBClearChart_clicked()
{
    illuLine->clear();
    tempLine->clear();
    humiLine->clear();
